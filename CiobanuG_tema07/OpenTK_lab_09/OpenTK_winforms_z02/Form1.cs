...
namespace OpenTK_winforms_z02
{
    public partial class Form1 : Form
    {
        ...
        private float[] valuesPosition1 = new float[] { 0.0f, 50.0f, 0.0f, 1.0f }; // Poziție inițială
        private float[] valuesAmbient1 = new float[] { 0.1f, 0.1f, 0.1f, 1.0f };
        private float[] valuesDiffuse1 = new float[] { 1.0f, 1.0f, 1.0f, 1.0f };
        private float[] valuesSpecular1 = new float[] { 0.5f, 0.5f, 0.5f, 1.0f };
        ...
        protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
        {
            if (lightON && lightON_1)
            {
                switch (keyData)
                {
                    case Keys.W: valuesPosition1[1] += 1.0f; break; // Sus
                    case Keys.S: valuesPosition1[1] -= 1.0f; break; // Jos
                    case Keys.A: valuesPosition1[0] -= 1.0f; break; // Stânga
                    case Keys.D: valuesPosition1[0] += 1.0f; break; // Dreapta
                    case Keys.Q: valuesPosition1[2] -= 1.0f; break; // În spate
                    case Keys.E: valuesPosition1[2] += 1.0f; break; // În față
                }
                GlControl1.Invalidate(); // Re-randează scena
            }
        ...
        private void GlControl1_MouseMove(object sender, MouseEventArgs e) {
            ...
            if (statusMouseDown && lightON && lightON_1)
            {
                valuesPosition1[0] += e.X - mousePos.X; // Actualizare X
                valuesPosition1[2] += e.Y - mousePos.Y; // Actualizare Z
                mousePos = new Point(e.X, e.Y); // Actualizare poziție mouse
                GlControl1.Invalidate(); // Re-randează scena
            }
        }
        ...
        // EXERCITIUL SURSA A DOUA-A DE ILUMINARE
        ...
        {
            valuesPosition1 = new float[] { 0.0f, 50.0f, 0.0f, 1.0f };
            valuesAmbient1 = new float[] { 0.1f, 0.1f, 0.1f, 1.0f };
            valuesDiffuse1 = new float[] { 1.0f, 1.0f, 1.0f, 1.0f };
            valuesSpecular1 = new float[] { 0.5f, 0.5f, 0.5f, 1.0f };
        }
